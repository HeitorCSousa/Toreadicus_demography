...
# install.packages("inlabru",dep=T)
# install.packages("INLA",dep=T)
...
m.Toreadicus.sprint <- gamm4(Veloc ~ t2(temp) + CRC,
  random = ~ (1 | PEL),
...
## Attach mean predictions and erros to dataframe
preddata_tpc_Toreadicus$predicted <- pred_tpc_Toreadicus$fit
preddata_tpc_Toreadicus$se <- pred_tpc_Toreadicus$se.fit
preddata_tpc_Toreadicus$lower <- pred_tpc_Toreadicus$fit - pred_tpc_Toreadicus$se.fit
preddata_tpc_Toreadicus$upper <- pred_tpc_Toreadicus$fit + pred_tpc_Toreadicus$se.fit

ggplot(preddata_tpc_Toreadicus, aes(x = temp, y = predicted)) +
  geom_line() +
  geom_point(data = dados.completos[dados.completos$especie == "T_oreadicus", ], aes(x = temp, y = Veloc), alpha = 0.5) +
  geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 2, alpha = 0.1) +
  labs(title = "Tropidurus oreadicus", y = "Predicted Speed (cm/s)", x = "Temperature (°C)") +
  lims(x = c(10, 45), y = c(0, 2.25)) +
  theme(plot.title = element_text(hjust = 0.5))

# Optimal temperature
preddata_tpc_Toreadicus[preddata_tpc_Toreadicus$predicted == max(preddata_tpc_Toreadicus$predicted), ]
# 28.1°C

# Predict locomotor performance for microclimate data
env.vars$Toreadicus_perf <- predict.gam(m.Toreadicus.sprint$gam,
...
env.vars.month <- subset(env.vars.month,
...
Toreadicus.CHxy <- make.capthist(capts, traps.xy,
  fmt = "XY",
  noccasions = 46
)
...
cjs.t <- openCR.fit(Toreadicus.CHxy,
...
cjs.full <- openCR.fit(Toreadicus.CHxy,
...
cjs.fulladd <- openCR.fit(Toreadicus.CHxy,
  model = list(phi ~ -1 + sexo + year, p ~ -1 + sexo + year),
  method = "BFGS",
  details = list(control = list(
    maxit = 100000,
...
cjs.psextphisex <- openCR.fit(Toreadicus.CHxy,
...
cjs.psexphisext <- openCR.fit(Toreadicus.CHxy,
...
cjs.ptphisext <- openCR.fit(Toreadicus.CHxy,
...
cjs.psextphit <- openCR.fit(Toreadicus.CHxy,
...
cjs.psexphit <- openCR.fit(Toreadicus.CHxy,
...
cjs.ptphisex <- openCR.fit(Toreadicus.CHxy,
...
cjs.sex <- openCR.fit(Toreadicus.CHxy,
...
# JAGS Pradel and CJS models ----------------------------------------------

# Use monthly data

# Filter variables of interest
table1 <- Toreadicus.planilha.XY[Toreadicus.planilha.XY$recaptura != "(s)", c("numerodetombo", "campanha", "sexo", "crc", "massa", "recaptura", "armadilha")]
str(table1)
...
218 + (2 * 69) + (3 * 37) + (4 * 17) + (5 * 13) + (6 * 7) + (7 * 4) + (8 * 3) + 11 + 12 + 13 
...
datA <- data.frame(table5$campanha, table5$sexo, table5$numerodetombo, table5$crc, table5$massa, Age, table5$armadilha)
...
eh <- cast(datA, TrueID ~ Year, fun.aggregate = function(x) as.numeric(length(x) > 0), value = "CRC")
eh <- eh[, 2:ncol(eh)]
...
sex <- cast(datA, TrueID ~ ., value = "Sex", fun.aggregate = function(x) tail(x, 1))
table(sex$`(all)`)
...
sex.months <- cbind(sex.months, c(rep(2018, 11), rep(2019, 12), rep(2020, 12), rep(2021, 11)))
...
# Package data
bugs.data <- list(
  u = u, n = n, v = v, d = d, first = f, ind = ind, nind = dim(eh)[1], n.occasions = dim(eh)[2],
  y = eh, z = known.states.cjs(eh), time = time,
...
inits <- function() {
  list(
    tauphib = 1, betaTphi = rep(0, 4), varphi = rep(0, 4),
    tauphibJS = 1, betaTphiJS = rep(0, 8), varphiJS = rep(0, 8),
    sigma.phiJS = runif(1, 1, 2),
    sigma.f = runif(1, 0.75, 1.5),
    taufb = 1, betaTf = rep(0, 8), varf = rep(0, 8),
    taupb = 1, betaTp = rep(0, 8), varp = rep(0, 8),
    taupbJS = 1, betaTpJS = rep(0, 8), varpJS = rep(0, 8),
...
PJS.df <- as.data.frame(pradel.Toreadicus.df[-c(47:64, 110:127, 174:191, 237), ])
head(PJS.df)
tail(PJS.df)
...
names(PJS.df) <- c(
  "mean", "sd", "low2.5", "low.25", "median",
  "hi75", "hi97.5", "Rhat", "n.eff", "overlap0",
  "f", "time", "date", "month", "year", "param"
)
...
meses <- month(seq(as.POSIXlt("2018-02-01"), as.POSIXlt("2021-11-30"), "month"), label = T, abbr = T)

# Plot results

# Survival (phi)
ggplot(PJS.df[PJS.df$param == "phi", ], aes(time)) +
  geom_rect(data = rect1, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = "#bdbdbd", inherit.aes = F) +
  geom_rect(data = rect2, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = "#bdbdbd", inherit.aes = F) +
  geom_rect(data = rect3, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = "#bdbdbd", inherit.aes = F) +
  geom_rect(data = rect4, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = "#bdbdbd", inherit.aes = F) +
  geom_rect(data = rect5, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = "#bdbdbd", inherit.aes = F) +
  ...
  scale_x_continuous(breaks = c(1:45), labels = meses[-46], expand = c(0, 0.6)) +
  labs(x = NULL, y = "Survival") +
  theme(plot.margin = unit(c(0, 0, 2, 0), "lines"), text = element_text(size = 10))
...
round(exp(pradel.Toreadicus.df["alpha.phiJS", "mean"]) / (1 + exp(pradel.Toreadicus.df["alpha.phiJS", "mean"])), 3) # Mean
round(exp(pradel.Toreadicus.df["alpha.phiJS", "2.5%"]) / (1 + exp(pradel.Toreadicus.df["alpha.phiJS", "2.5%"])), 3) # Low CI
round(exp(pradel.Toreadicus.df["alpha.phiJS", "97.5%"]) / (1 + exp(pradel.Toreadicus.df["alpha.phiJS", "97.5%"])), 3) # High CI

phi.df <- PJS.df[PJS.df$param == "phi", ]

# Mean of capture
pradel.Toreadicus.df["alpha.pJS", "mean"]
round(exp(pradel.Toreadicus.df["alpha.pJS", "mean"]) / (1 + exp(pradel.Toreadicus.df["alpha.pJS", "mean"])), 3) # Mean
round(exp(pradel.Toreadicus.df["alpha.pJS", "2.5%"]) / (1 + exp(pradel.Toreadicus.df["alpha.pJS", "2.5%"])), 3) # Low CI
round(exp(pradel.Toreadicus.df["alpha.pJS", "97.5%"]) / (1 + exp(pradel.Toreadicus.df["alpha.pJS", "97.5%"])), 3) # High CI
...
(sens.phi <- colMeans(pradel.Toreadicus$sims.list$phiJS[, -46] / pradel.Toreadicus$sims.list$rho[, -1]))

(sens.phi2.5 <- apply((pradel.Toreadicus$sims.list$phiJS[, -46] / pradel.Toreadicus$sims.list$rho[, -1]), 2, quantile, probs = 0.025))
(sens.phi97.5 <- apply((pradel.Toreadicus$sims.list$phiJS[, -46] / pradel.Toreadicus$sims.list$rho[, -1]), 2, quantile, probs = 0.975))


(sens.f <- colMeans(pradel.Toreadicus$sims.list$f[, -46] / pradel.Toreadicus$sims.list$rho[, -1]))

(sens.f2.5 <- apply((pradel.Toreadicus$sims.list$f[, -46] / pradel.Toreadicus$sims.list$rho[, -1]), 2, quantile, probs = 0.025))
(sens.f97.5 <- apply((pradel.Toreadicus$sims.list$f[, -46] / pradel.Toreadicus$sims.list$rho[, -1]), 2, quantile, probs = 0.975))
...
sens.df <- data.frame(sens.phi, sens.phi2.5, sens.phi97.5,
  sens.f, sens.f2.5, sens.f97.5,
  month = phi.df$month,
  time = phi.df$time
)
...
ggplot(sens.df, aes(time, sens.phi)) +
  ...
  scale_x_continuous(breaks = c(1:45), labels = meses[-46], expand = c(0, 0.6)) +
  geom_ribbon(aes(ymin = sens.phi2.5, ymax = sens.phi97.5), fill = "gold", alpha = .2) +
  geom_ribbon(aes(ymin = sens.f2.5, ymax = sens.f97.5), fill = "blue", alpha = .2) +
...
(meses <- c(rep(c("Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Jan"), 3), "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov"))

head(dados)

plot(crc ~ campanha,
  data = Toreadicus.data,
  ylab = "Snout-vent length (mm)", col = rgb(0, 0, 0, .25), pch = 19,
  axes = F, cex = 1.5, bty = "n", ylim = c(20, 100), xlab = "Months"
)
...
boxplot(crc ~ month,
  data = Toreadicus.data,
  axes = F, ylab = "Snout-vent length (mm)", ylim = c(20, 100), xlab = "Months"
)
axis(1, 1:12, labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))
axis(2)
abline(h = 50, lty = 3)
...
barplot(as.matrix(t(cap.recap.months.To.PEL[, -1])),
  names.arg = meses,
  legend.text = T,
  ylim = c(0, 50),
  ylab = "Frequency",
...
barplot(arms.caps.To.PEL$capturas,
  names.arg = 1:25,
  col = c(
    rep("#2c7bb6", 6),
    rep("#abd9e9", 6),
...
Toreadicus.captures.day.env <- left_join(Toreadicus.captures.day,
  env.vars.month,
  by = c("trap", "t")
)
...
out.inla.0 <- inla(counts.and.count.covs[, c(1:7)] ~ 1,
...
out.inla.1 <- inla(counts.and.count.covs ~ 1 + x1.p + x2.p,
  data = list(
...
mesh.inla <- inla.mesh.2d(boundary = bnd, arms.pts.PEL.utm, max.edge = c(30, 100), cutoff = 30)
...
out.inla.4 <- inla(
  counts.and.count.covs ~ 1 + x1.p + x2.p + x1.p:x2.p +
    f(spatial,
      model = spde,
      A.local = A,
      group = Toreadicus.captures.day.env$t,
...
counts.and.count.covs <- inla.mdata(
  ...
  scale(Toreadicus.captures.day.env$precip),
  scale(Toreadicus.captures.day.env$Toreadicus_perf),
  scale(Toreadicus.captures.day.env$precip) * scale(Toreadicus.captures.day.env$Toreadicus_perf)
)
...
waic_table$formula[3] <- "(lambda ~ precip + perf), (p ~ precip + perf + spatiotemporal + temporal)"
waic_table$formula[4] <- "(lambda ~ precip + perf), (p ~ precip + perf + spatiotemporal)"
waic_table$formula[5] <- "(lambda ~ precip + perf), (p ~ precip * perf + spatiotemporal)"
waic_table$formula[6] <- "(lambda ~ 1), (p ~ spatiotemporal)"
waic_table$formula[7] <- "(lambda ~ precip * perf), (p ~ precip * perf + spatiotemporal)"
waic_table$formula[8] <- "(lambda ~ precip * perf), (p ~ precip + perf + spatiotemporal)"
...
(theta <- 1 / out.inla.7$summary.hyperpar["overdispersion for NMixNB observations", "mean"])
...
par(mfrow = c(1, 1))
plot(jitter(rowMeans(y.mat)), out.inla.7$residuals$deviance.residuals,
  bty = "n",
...
plot(fitted_values, out.inla.7$residuals$deviance.residuals,
  bty = "n",
...
plot(fitted_values, out.inla.7$cpo$cpo,
  bty = "n",
  main = "CPO vs. Fitted",
  xlab = "Fitted Values",
  ylab = "CPO-values"
...
plot(jitter(rowMeans(y.mat)), out.inla.7$cpo$cpo,
  bty = "n",
...
plot(jitter(rowMeans(y.mat)), fitted_values,
  bty = "n",
...
theoretical_nb <- qnbinom(ppoints(length(out.inla.4$residuals$deviance.residuals)), mu = mean(y.mat), size = theta)

# QQ-plot for negative binomial residuals
qqplot(theoretical_nb, out.inla.7$residuals$deviance.residuals, main = "QQ-Plot vs Negative Binomial", xlab = "Theoretical Quantiles", ylab = "Pearson Residuals") +
  theme_classic()


# Predictive R² (squared Pearson correlation)
cor(rowMeans(y.mat), fitted_values)^2
cor(rowMeans(y.mat), fitted_values,
  method = "spearman"
)^2
...
ggplot(
  stack(out.inla.7.lambda.fits[, c("mean.lambda", "median.lambda", "mode.lambda")]),
...
ggplot(
  stack(log(out.inla.7.lambda.fits[, c("mean.lambda", "median.lambda", "mode.lambda")])),
  aes(x = values, fill = ind)
) +
  geom_density(alpha = 0.5, col = NA) +
  labs(y = "Density", x = "log(Abundance)") +
...
ggplot(field_t, aes(x = x / 1000, y = y / 1000, fill = exp(z))) +
  ...
  geom_point(
    data = pred_df,
    aes(x = x / 1000, y = y / 1000, colour = N), fill = NA
  ) +
...
ggplot(field_t, aes(x = x / 1000, y = y / 1000, fill = z)) +
  ...
  geom_point(
    data = pred_df,
    aes(x = x / 1000, y = y / 1000, colour = p), fill = NA
  ) +
  scale_color_viridis_c(trans = "log10") +
  facet_wrap(~t) +
  labs(x = "Latitude (km)", y = "Longitude (km)", fill = "Spatial field") +
...
rel_p_precip$p <- plogis(out.inla.7$summary.fixed$`0.5quant`[1] +
  out.inla.7$summary.fixed$`0.5quant`[2] * rel_p_precip$precip)

rel_p_precip$plower <- plogis((out.inla.7$summary.fixed$`0.975quant`[1]) +
  (out.inla.7$summary.fixed$`0.025quant`[2]) * rel_p_precip$precip)

rel_p_precip$pupper <- plogis((out.inla.7$summary.fixed$`0.025quant`[1]) +
  (out.inla.7$summary.fixed$`0.975quant`[2]) * rel_p_precip$precip)

ggplot(pred_df, aes(x = precip, y = p)) +
  geom_point(size = 3, alpha = 0.3) +
  geom_line(data = rel_p_precip, aes(x = precip, y = p), col = "blue", linewidth = 2) +
  geom_ribbon(data = rel_p_precip, aes(ymin = plower, ymax = pupper), alpha = 0.3) +
...
rel_p_perf$p <- exp(out.inla.7$summary.fixed$`0.5quant`[1] +
  out.inla.7$summary.fixed$`0.5quant`[3] * rel_p_perf$perf)

rel_p_perf$plower <- exp((out.inla.7$summary.fixed$`0.025quant`[1]) +
  (out.inla.7$summary.fixed$`0.025quant`[3] * rel_p_perf$perf))

rel_p_perf$pupper <- exp((out.inla.7$summary.fixed$`0.975quant`[1]) +
  (out.inla.7$summary.fixed$`0.975quant`[3] * rel_p_perf$perf))

ggplot(pred_df, aes(x = perf, y = p)) +
  geom_point(size = 3, alpha = 0.3) +
  geom_line(data = rel_p_perf, aes(x = perf, y = p), col = "blue", linewidth = 2) +
  geom_ribbon(data = rel_p_perf, aes(ymin = plower, ymax = pupper), alpha = 0.3) +
...
rel_N$N <- exp(out.inla.7$summary.hyperpar$`0.5quant`[1] + # Intercept
  ...
  out.inla.7$summary.hyperpar$`0.5quant`[4] * rel_N$interaction) # Interaction
...
persp(
  ...
  col = pal[facetcol],
  theta = 45, phi = 30,
  xlab = "Traps", ylab = "Months", zlab = "Abundance"
)
...
persp(
  ...
  col = pal[facetcol],
  theta = 45, phi = 30,
  xlab = "Traps", ylab = "Months", zlab = "Capture"
)
...
perf.locfit <- locfit(Toreadicus_perf ~ lp(month.GMT_3, trap), data = Toreadicus.captures.day.env)
plot(perf.locfit)
...
ncaps.locfit <- locfit(freq ~ lp(month.GMT_3, trap), data = Toreadicus.captures.env)
plot(ncaps.locfit)
...
plot(abund, type = "b", pch = 19, bty = "n", axes = F, xlab = "", ylab = "Captures and recaptures", ylim = c(0, 50))
axis(1, 1:46, labels = meses)
axis(2)
...
plot(abund.df$Nmean[-46], phi.df$mean,
  ylab = "Survival",
  xlab = "Abundance", bty = "n", xlim = c(0, 200), ylim = c(0.6, 0.8),
  pch = 19, col = rgb(0, 0, 0, .5)
)
...
summary(m1)
lines(1:200, predict(m1, newdata = data.frame(N = 1:200), type = "response"), col = "red")
...
cor.test(log(abund.df$Nmix), log(PJS.df$mean[PJS.df$param == "p"]), method = "spearman")
...
plot(log(f) ~ Nmix,
  data = ddrecruit.df, ylab = "Recruitment",
  xlab = "Abundance", bty = "n", ylim = c(-3, 2),
  pch = 19, col = rgb(0, 0, 0, .5)
)
...
plot(log(f) ~ N,
  data = ddrecruit.df, ylab = "Recruitment",
  xlab = "Abundance", bty = "n", xlim = c(0, 200), ylim = c(-3, 2),
  pch = 19, col = rgb(0, 0, 0, .5)
)
...
summary(m2)
lines(1:200, predict(m2, newdata = data.frame(N = 1:200), type = "response"), col = "red")
...
acfncaps <- acf(tapply(Toreadicus.captures.env$freq, Toreadicus.captures.env$t, sum), lag.max = 30, plot = F)

plot(acfncaps,
  bty = "n", xlim = c(0, 30), main = "Captures"
)
...
plot(acfnmean,
  bty = "n", xlim = c(0, 30), main = "Abundance - PJS"
)
...
plot(acfnmix,
  bty = "n", xlim = c(0, 30), main = "Abundance - N-mixture"
)
...
plot(ccf.nmixpjs,
  bty = "n", main = "Cross-correlation function between abundance estimates"
)
...
# Cria objeto de série temporal (ts)
ts_rho <- ts(log(PJS.df[PJS.df$param == "rho", "mean"]),
  start = c(2018, 3),
  freq = 12
)
...
# Cria objeto de série temporal (ts)
ts_f <- ts(log(PJS.df[PJS.df$param == "f", "mean"]),
  start = c(2018, 3),
  freq = 12
)
...
# Cria objeto de série temporal (ts)
ts_phi <- ts(qlogis(PJS.df[PJS.df$param == "phi", "mean"]),
  start = c(2018, 3),
  freq = 12
)
...
# Cria objeto de série temporal (ts)
ts_pPJS <- ts(PJS.df[PJS.df$param == "p", "mean"],
  start = c(2018, 2),
  freq = 12
)
...
plot(jitter(freq) ~ precip,
  pch = 19, col = rgb(0, 0, 0, alpha = .2), data = Toreadicus.captures.env,
  bty = "n", xlab = "Precipitation (m)", ylab = "Abundance", xlim = c(0, 200), ylim = c(0, 14)
)

plot(freq ~ Toreadicus_perf,
  pch = 19, col = rgb(0, 0, 0, alpha = .2), data = Toreadicus.captures.env,
  bty = "n", xlab = "Locomotor performance", ylab = "Abundance", xlim = c(0.8, 1.8), ylim = c(0, 14)
)

plot(Toreadicus_perf ~ Toreadicus_ha90,
  pch = 19, col = rgb(0, 0, 0, alpha = .2), data = Toreadicus.captures.env,
  bty = "n", xlab = "Hours of activity", ylab = "Locomotor performance", ylim = c(0.8, 1.8)
)

plot(Toreadicus_perf ~ tmed,
  data = Toreadicus.captures.env, pch = 19, col = rgb(0, 0, 0, alpha = .2),
  bty = "n", xlab = "Mean temperature (ºC)", ylab = "Locomotor performance", ylim = c(0.8, 1.8), xlim = c(20, 28)
)

plot(Toreadicus_perf ~ rhmax,
  data = Toreadicus.captures.env, pch = 19, col = rgb(0, 0, 0, alpha = .2),
  bty = "n", xlab = "Relative humidity (%)", ylab = "Locomotor performance", ylim = c(0.8, 1.8), xlim = c(65, 100)
)
...
APASL <- CUs[CUs$name_conservation_unit == "ÁREA DE PROTEÇÃO AMBIENTAL SERRA DO LAJEADO", ]
...
inset.cerrado <- ggplot() +
  ...
  geom_rect(aes(xmin = -49, xmax = -47.5, ymin = -11, ymax = -9), color = "red", fill = NA) +
  theme_test() +
  theme(
...
inset.pel <- ggplot() +
  ...
  geom_rect(aes(xmin = -48.215, xmax = -48.200, ymin = -10.152, ymax = -10.140), color = "red", fill = NA) +
  theme_test() +
  theme(
    axis.text = element_blank(),
    axis.ticks = element_blank(),
...
imagery <- get_tiles(bbox.arms, crop = TRUE, provider = "Esri.WorldImagery", zoom = 16)
plot(imagery)
...
map.arms.pel <- ggplot() +
  ...
  geom_sf_text(data = arms.pts.PEL, aes(label = armadilha), nudge_x = 0.0003, nudge_y = 0.0003, size = 2.5, color = "red") +
  xlim(-48.212, -48.202) +
  ylim(-10.150, -10.142) +
  theme_test() +
  guides(size = "none") +
  ...
  coord_sf(xlim = c(-48.212, -48.203), ylim = c(-10.150, -10.143), expand = FALSE) +
  labs(x = "Longitude", y = "Latitude")
